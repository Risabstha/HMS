...
<body>
  ...
  function generate_bill()
  {
    ...
    $query = mysqli_query($con, "
        SELECT DISTINCT p.pid, p.ID, p.fname, p.lname, p.doctor, p.appdate, p.apptime, p.disease, p.allergy, p.prescription, a.docFees 
        FROM prestb p 
    ...
    while ($row = mysqli_fetch_array($query)) {
      if (isset($displayed[$row['ID']])) {
        continue; // Skip duplicates
      }
      $displayed[$row['ID']] = true;
  ...
  ';
    ...
    // Ensure generate_bill() returns only required content
